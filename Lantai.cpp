...
Lantai::Lantai(Shader* shader, Texture* diffuseTexture, Texture* specularTexture)
{
	this->shader = shader;
...
void Lantai::InitData()
{
	...
	float vertices[] = {
		...
		-0.5, 0.5, -0.5,   1, 1,   0.0f,  1.0f,  0.0f, // 18
		 ...
		 -0.5, -0.5,  0.5,  0, 1,   0.0f,  -1.0f,  0.0f, // 23a
...
void Lantai::InitTexture()
{
	diffuseTexture->Init();
	specularTexture->Init();    // Use Load() instead of Init() if your Texture class has a Load() function.
}
...
void Lantai::SetPosition(float x, float y, float z)
{
	position.x = x;
	position.y = y;
	position.z = z;
...
void Lantai::SetScale(float sx, float sy, float sz)
{
	scale.x = sx;
	scale.y = sy;
	scale.z = sz;
...
void Lantai::SetRotation(float angle, float x, float y, float z)
{
	this->angle = angle;
	rotationAxis.x = x;
	rotationAxis.y = y;
